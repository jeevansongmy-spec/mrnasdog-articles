...
description: "A MrNasdog Pressure Framework read of UNUS SED LEO: no mint, no vesting, only a revenue-funded buyback-and-burn. Framework −0.07% net; the supply monitor agrees at −0.10%."
...
UNUS SED LEO, the Bitfinex exchange token issued by iFinex, has no mint function, no vesting calendar and exactly one supply mechanism: a buyback-and-burn funded by a share of exchange revenue. Over the 90 days to **Aug 9 2026** that mechanism retired **633,782 LEO** against **zero** new issuance, so the MrNasdog Pressure Framework reads LEO at **−0.07% net** on a **920.0M** float. Our independent supply monitor reads **−0.10%** across the same window — a gap of **0.03 percentage points**, which is agreement, not conflict.

## The verdict, in one paragraph

For the 90-day window ending **Aug 9 2026**, the Pressure Framework reads LEO at **−0.07% net**: every sell row in the LEO ledger is zero, and the only live buy row is the iFinex buyback-and-burn, which removed **633,782 LEO** from the circulating float. Our supply monitor reads the realised change at **−0.10%** over the same window, and the gap between the two readings is **0.03 percentage points** — far inside the framework's half-point tolerance, so LEO ships with **no monitor-gap flag**. The label for LEO is **deflationary by contract, but slowly**: the supply can only ever fall, and at the current pace it falls by roughly a quarter of a percent a year. The burn is real, verifiable on-chain, and small.
