---
title:         "XDC Inflation Analysis · August 2026 · Supply flat, projected to stay roughly flat"
description:   "XDC's 108 masternodes mint ~18.94M XDC/90d at 5.5 per block, with no buyback and a sub-cent fee burn. Framework reads +0.09% net (monitor −0.15%)."
...
XDC Network mints about **18.94M XDC** every 90 days as masternode block rewards, and almost nothing removes any of it. The MrNasdog Pressure Framework reads **+0.09% net** on a **19.95B** circulating base against no hard cap — the XDC block reward is the only real new-supply force, and the independent monitor reads the chain as essentially flat at **−0.15%**.

## The verdict, in one paragraph

For the 90-day window ending **Aug 5 2026**, the framework reads **XDC at +0.09% net inflation** — masternode block-reward emission alone, with a buy ledger that is empty in practice. The independent monitor reads **−0.15%** over the same window, a gap of **0.25 percentage points**, well inside the half-point tolerance, so no monitor-gap flag is raised. XDC is a quiet, uncapped delegated-proof-of-stake chain: a small, steady mint on a very large float, structurally inflationary but only just.

## Sell pressure: where new XDC comes from

One source carries the whole reading. Sell #1, protocol inflation, booked **~18.94M XDC**. The XDC Network pays masternodes a fixed **5.5 XDC** per block, and this is newly minted supply rather than a payout from a reserve — the chain has no fixed cap, so every block reward grows total supply. The number here is measured on-chain, not assumed: over the window the chain moved from block 102,300,000 to block 105,745,380, which is **3,445,380** blocks in **7,779,814 seconds**, an actual block time of **2.26 seconds** rather than the 2-second nominal. At that real rate a 90-day window produces about 18.94M XDC, roughly 77M a year, a touch below the 86.7M a nominal 2-second block would imply.
...
The XDC buy ledger is empty in every line that matters. Buy #1, programmatic buyback, is **0**: neither the protocol nor the foundation runs a contract that repurchases XDC. Buy #2, protocol fee burn, is **0** in effect: XDC actually carries two burns — an EIP-1559 base-fee burn added in the early-2026 Cancun upgrade, and a long-standing 20% burn on smart-contract fees — but XDC fees are a fraction of a cent, so the amount destroyed across 90 days is well under 0.5M XDC. The proof is in the supply itself: total supply has grown at essentially the gross mint rate, which means burns are barely registering. Buy #3, Foundation buy, is **0**, with no announced accumulation programme. Buy #4, new long-term lock, is **0**: the masternode set held at 108 validators, so no new collateral was locked, and masternode stake can be withdrawn anyway. New XDC enters; almost nothing is taken back out.
