...
description: "Stellar mints nothing since 2019, yet XLM float still grows +2.75% per 90 days as the Foundation releases ~946.3M XLM from its mandate wallets. Monitor +2.80%."
...
Stellar mints nothing. The XLM protocol inflation mechanism was retired by a validator vote on Oct 28 2019 and total supply has been fixed near **50.00B XLM** ever since — yet the Pressure Framework still reads **+2.75%** net supply reaching the market over 90 days, closely matching the monitor's **+2.80%**. The reason is that every lumen entering circulation comes from one place: the Stellar Development Foundation releasing XLM out of its own mandate wallets, **~946.3M XLM** of it this window. Stellar is a fixed-supply network with a discretionary float.

## The verdict, in one paragraph

The framework books **~946.3M XLM** of sell pressure and **~0.59M XLM** of buy pressure against a circulating base of **~34,413.2M XLM**, for a net of **+2.75%** over the trailing 90 days and the same **+2.75%** projected for the next 90. The inflation monitor independently reads **+2.80%** for the same window, so the gap is just **0.06pp** — comfortably inside tolerance, no data-conflict chip, no deep walk required. The two numbers agree because they are measuring the same physical event from opposite ends: the monitor watches circulating supply rise, and the framework watches the Stellar Development Foundation wallets that supply it fall. The cite-able label for XLM is this — **a fixed-supply chain that is structurally inflationary on its active float**. The dilution is real and it is roughly 2.75% a quarter, but it is a treasury decision, not a protocol rule, and that distinction is the entire investment case either way.

## Sell pressure: where new XLM comes from

Sell #1, protocol inflation, is **0**. Stellar ran a 1% annual inflation mint for roughly five years, generating **5,443,902,087 XLM** before validators voted it off on Oct 28 2019; the following week the Stellar Development Foundation sent about **55.44B XLM** to a locked address, cutting total supply from ~105B to ~50B. Reading the Stellar ledger directly at both ends of this window returns an identical total-coins figure of **105,443,902,087 XLM**, so the mint is genuinely off — not slowed, off. It is worth being precise about the tag: because inflation was disabled by a validator vote rather than removed by immutable code, another validator vote could in principle restore it, so the framework keeps this row watched rather than permanently closed.
...
Sell #3, Foundation and unscheduled unlocks, is **~946.3M XLM** — effectively 100% of Stellar's sell pressure. The Stellar Development Foundation publishes 13 named mandate accounts across buckets including Direct Development, Stellar Growth, and Product and Innovation. Reading all 13 accounts on-chain and walking their native transaction effects across the 90-day window shows a net release of **~946.3M XLM** into the float once internal foundation-to-foundation transfers cancel inside the perimeter, leaving about **15,301.4M XLM** still held. This is on-chain ground truth, not an estimate from a schedule, and it lines up with the monitor's independent **+938.9M** rise in circulating supply — two methods within about 1% of each other. The pace held at roughly **10.5M XLM a day**, steady with the spring rather than accelerating, so the reading stayed in the same band rather than tipping into a higher one.
...
Buy #4, new long-term lock, is **~0.59M XLM** — the one genuine buy-side entry on the page, and it is the fee pool. Because the pool belongs to no account and no one holds keys to it, lumens that land there are permanently removed from circulation: economically a lock, not a burn, which is why the framework books it here rather than under Buy #2. Reading the ledger's fee-pool field at both ends of the window shows growth from about **9.74M XLM** to about **10.33M XLM**. It is a real offset. It is also roughly 1/1600th the size of the sell side, so it changes nothing about the verdict.

## Foundation and overhang

XLM has exactly one team-controlled overhang, and it is enormous. The Stellar Development Foundation's 13 mandate accounts still hold about **15,301.4M XLM** — roughly **31% of total supply** and about **44% of the current float** — spread across its Direct Development, Stellar Growth, and Product and Innovation buckets. There is no published release calendar for any of it: the mandate describes purposes, not dates or quanta. At the realised rate of the last 90 days this overhang would take roughly four more years to exhaust, but nothing obliges the Stellar Development Foundation to move at that pace, in either direction.
